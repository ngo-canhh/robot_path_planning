...
class RRTPathPlanner:
    def __init__(self, width, height, robot_radius, max_iterations=1000, step_size=20, goal_sample_rate=0.2):
        self.width = width
        self.height = height
        self.robot_radius = robot_radius
    ...
    def plan_path(self, start_x, start_y, goal_x, goal_y, obstacles):
        ...
        for _ in range(self.max_iterations):
            if np.random.random() < self.goal_sample_rate:
                ...
                sample_x = np.random.uniform(0, self.width)
                sample_y = np.random.uniform(0, self.height)

            nearest_idx = self._find_nearest_node(sample_x, sample_y)
            nearest_node = self.nodes[nearest_idx]
            new_node = self._steer(nearest_node, (sample_x, sample_y))

            if self._is_segment_collision_free(nearest_node, new_node, obstacles):
                self.nodes.append(new_node)
                self.parents.append(nearest_idx)

                if self._is_goal_reached(new_node, goal_x, goal_y):
                    goal_reached = True
        ...
        return path, self.nodes, self.parents

    def _find_nearest_node(self, x, y):
        distances = [np.sqrt((node[0] - x)**2 + (node[1] - y)**2) for node in self.nodes]
    ...
    def _steer(self, from_node, to_point):
        ...
        new_distance = min(self.step_size, distance)
        new_node = np.array(from_node) + new_distance * direction
        return (new_node[0], new_node[1])

    def _is_segment_collision_free(self, from_node, to_node, obstacles):
        for obstacle in obstacles:
            if obstacle.shape.intersects_segment(from_node, to_node, self.robot_radius, obstacle.x, obstacle.y):
                return False
        return True

    def _is_goal_reached(self, node, goal_x, goal_y):
        distance = np.sqrt((node[0] - goal_x)**2 + (node[1] - goal_y)**2)
        return distance < self.robot_radius + 5